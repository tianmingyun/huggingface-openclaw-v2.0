...
### 问题一：skill 安装后重启就消失（已修复）

#### 先理解什么是 skill

Skill 是 OpenClaw 的"专业技能包"。比如：
- 你安装了一个"图片生成"skill，OpenClaw 就能帮你画图
- 你安装了一个"代码审查"skill，OpenClaw 就能帮你检查代码
- 你安装了一个"天气查询"skill，OpenClaw 就能查天气

这些 skill 可以通过聊天窗口直接安装，非常方便。

#### 以前发生了什么？

假设今天下午你安装了 3 个 skill：
- 14:00 安装了"图片生成"
- 15:30 安装了"代码审查"
- 17:00 安装了"天气查询"

你们聊得很开心，OpenClaw 帮你完成了很多工作。到了晚上，服务器需要重启（或者你手动重启了）。
...
OpenClaw 突然"失忆"了，不记得自己会画图、会审查代码、会查天气了。你得重新安装一遍。
...
这就像是搬家时，你打包了衣服、书籍、电器，却忘了打包工具箱。到了新家，工具全没了。

#### 现在怎么解决的？

非常简单：**我们把 skills 目录加进了备份列表。**
...
同时，变化检测系统也会监控 skills 目录。一旦你安装或修改了 skill，5 分钟内系统就会检测到变化并自动备份。

**效果：** 现在安装 skill → 系统自动备份 → 重启服务器 → skill 完好无损。
...
假设你刚刚完成了一个非常重要的工作，想立刻备份。你在聊天窗口输入：
...
或者你直接执行命令：
```bash
python3 /home/node/app/backup_controller.py manual
```

**结果可能是：**
```
[BACKUP-CONTROL] Skipping backup, too soon (reason: manual)
```

翻译一下："备份跳过了，因为距离上次备份太近（原因是手动触发）。"

**为什么会这样？**

因为系统有一个"防重复保护"机制：5 分钟内只能备份一次。这个机制本意是好的——防止系统疯狂备份浪费资源。但它一视同仁，连"手动立即备份"也挡住了。

想象一下：你刚拍完一张重要照片，想立刻上传到云盘，结果云盘说"你 3 分钟前上传过一张，请等 2 分钟再来"。虽然理解它的用意，但确实很让人着急。
...
**核心改动：**
- 自动备份（检测到变化触发的）→ 仍然遵守 5 分钟间隔，保护系统资源
- 手动备份（你主动说"立即备份"）→ 强制执行，不受 5 分钟限制

就像医院急诊和普通门诊的区别：普通病人按号排队（自动备份），急诊病人优先处理（手动备份）。

**效果：** 现在你说"立即备份"，它就真的立即备份，不会拒绝。



本项目是基于 OpenClaw 2026.4.2 架构重构的 v2 版本部署方案。支持多供应商大模型热插拔、主从 Agent 任务分发，以及容器重启状态持久化。支持通过factory rebuild方式更新到最新版

最新更新：

### 1. **更智能的备份时机**（从"定时"到"按需"）
...
#### ⏰ 2小时保险机制（兜底策略）
如果**连续2小时没有任何变化**（比如你睡觉了），系统会强制备份一次，确保数据安全。

#### 🛡️ 防重复保护
如果5分钟内已经备份过了，即使有变化也不会重复备份，避免浪费资源。
...
**关于存储空间：**
备份存储在Hugging Face的Dataset中（类似云盘），有容量限制。如果空间满了，你可以手动删除旧的备份（后面会教你怎么操作）。
